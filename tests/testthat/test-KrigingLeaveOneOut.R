...
for (kernel in c("gauss", "exp", "matern3_2", "matern5_2")) {
    ...
    y <- f(X)
    points(X, y)
    k <- DiceKriging::km(design = X, response = y,
                         covtype = kernel,
                         control = list(trace = FALSE))
    ll <- function(theta) DiceKriging::leaveOneOutFun(theta, k)
    
    plot(Vectorize(ll), ylab = "LL", xlab = "theta", xlim = c(0.01, 1))
    for (x in seq(from = 0.01, to = 1, length.out = 11)){
        envx <- new.env()
        llx <- DiceKriging::leaveOneOutFun(x, model = k, envir = envx)
        gllx <- DiceKriging::leaveOneOutGrad(x, model = k, envir = envx)
        arrows(x, llx, x + .1, llx + .1 * gllx)
    }
    
    r <- Kriging(y, X, kernel)
    ll2 <- function(theta) leaveOneOut(r, theta)$leaveOneOut[1]
    ## plot(Vectorize(ll2), col = 'red', add = TRUE)
    ## FIXME fails with "error: chol(): decomposition failed"
    for (x in seq(0.01,1,,11)){
        envx <- new.env()
        ll2x <- leaveOneOut(r,x)$leaveOneOut[1]
        gll2x <- leaveOneOut(r, x, grad = TRUE)$leaveOneOutGrad
    ...
    test_that(desc = "leaveOneOut is the same that DiceKriging one",
              expect_equal(leaveOneOut(r, x)$leaveOneOut[1],
                           DiceKriging::leaveOneOutFun(x, k, envir = xenv),
                           tolerance = precision))
    
    test_that(desc = "leaveOneOut Grad is the same that DiceKriging one",
              expect_equal(leaveOneOut(r,x,grad = T)$leaveOneOutGrad,
                           DiceKriging::leaveOneOutGrad(x, k, envir = xenv),
                           tolerance = precision))
}
