...
## XXXY not really useful in a R package
## refpath <- find_dir()

refpath <- system.file("libKriging/tests/references", package = "rlibkriging")
...
for (n in names(llo)) {
    ...
    if (do_write) {
        write.table(llo[[n]], file = filer,
                    row.names = FALSE, col.names = FALSE, sep = ',')
    }
    testme <- as.matrix(read.table(file = filer, header = FALSE, sep = ','))
    test_that(desc = paste0("Computed result should be  close to reference value on ",
                            prefix, " llo$", n), {
...
for (n in names(loglik)) {
  filer <- file.path(refpath, sprintf("%s-result-%s.csv", prefix, n))
  if (do_write) {
      write.table(loglik[[n]], file = filer,
                  row.names = FALSE, col.names = FALSE, sep = ',')
  }
  testme <- as.matrix(read.table(file = filer, header = FALSE, sep = ','))
  test_that(desc = paste0("Computed result should be close to reference value on ",
...
for (i in 1:length(logn)) {
    ...
    for (n in names(loglik)) {
        ...
        if (do_write) {
            write.table(loglik[[n]], file = filer, row.names = FALSE, col.names = FALSE,
                        sep = ',')
        }
        testme <- as.matrix(read.table(file = filer, header = FALSE, sep = ','))
        test_that(desc = paste0("Computed result should be close to reference value on ",
