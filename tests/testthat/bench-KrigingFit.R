...
if (FALSE) {
    pack <- list.files(file.path("bindings","R"), pattern = ".tar.gz",
                       full.names = TRUE)
...
for (i in 1:N) {
    ...
    times$R[i] <-  system.time(
        k <- DiceKriging::km(design = X, response = y, covtype = "gauss",
                             multistart = 1, control = list(trace = TRUE, maxit = 10))
        ##,lower=rep(0.001,d),upper=rep(2*sqrt(d),d))
    )
    times$cpp[i] = system.time(
        r <- Kriging(y, X, kernel = "gauss", regmodel = "constant", normalize = FALSE,
    ...
    if (abs(ll_cpp - DiceKriging::logLikFun(param = as.numeric(t(as.list(r)$theta)),
                                            model = k)) / ll_cpp>.1)
        stop("LL function is not the same bw DiceKriging/libKriging: ",
             logLikelihood(r,t(as.list(r)$theta))," vs. ",
             DiceKriging::logLikFun(param = as.numeric(t(as.list(r)$theta)), model = k))  
